...
const { attribute } = require('postcss-selector-parser')
 ...
 * @param {VElement} sibling
 * @param {number} lineDifference
 * @description Checks whether the element is conditional and whether the closest sibling is a v-else or v-else-if
 */
function isConditionalElementWithoutFallback(
  context,
  tag,
  sibling,
  lineDifference
  ...
  const endTag = tag.endTag || tag.startTag
  const isConditional =
    utils.hasDirective(tag, 'if') || utils.hasDirective(tag, 'else-if')

  const siblingIsExcluded =
    (!utils.hasDirective(sibling, 'if') ||
      !utils.hasDirective(sibling, 'else-if')) &&
    !utils.hasDirective(sibling, 'else')

  if (!isConditional) {
    insertNewLine(context, tag, sibling, lineDifference)
  } else if (isConditional && siblingIsExcluded) {
    insertNewLine(context, tag, sibling, lineDifference)
  } else {
    removeExcessLines(context, endTag, sibling, lineDifference)
  }
}
...
function checkNewline(context) {
  /** @type {Array<{blankLine: "always" | "never" | "consistent" | "non-conditional", prev: string, next: string, betweenConditionalTags: boolean}>} */
  const configureList = context.options[0] || [
    { blankLine: 'always', prev: '*', next: '*', betweenConditionalTags: true } // TODO: Set betweenConditionalTags to matching value of blankLine
  ]
  ...
  return (block) => {
    ...
    let { blankLine, betweenConditionalTags } = configure
    ...
    if (blankLine === 'always') {
      if (!betweenConditionalTags) {
        isConditionalElementWithoutFallback(
          context,
          block,
          closestSibling,
          lineDifference
    ...
    schema: [
      {
        ...
        items: {
          ...
          properties: {
            ...
            betweenConditionalTags: { type: 'boolean' },
            prev: { type: 'string' },
            next: { type: 'string' }
          },
